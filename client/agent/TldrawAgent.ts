...
/**
 * Rate limiter to ensure a minimum delay between API calls.
 * This helps prevent hitting rate limits by spacing out requests.
 */
class RateLimiter {
	private lastCallTime: number = 0
	private minDelayMs: number = 1000 // Minimum 1 second between calls (configurable)
	 ...
	 * If less than minDelayMs has passed, wait for the remaining time.
	 */
	async waitForNextCall(): Promise<void> {
		...
		if (timeSinceLastCall < this.minDelayMs) {
			...
			console.log(`[RateLimiter] Waiting ${waitTime}ms before next API call to avoid rate limiting`)
...
const apiRateLimiter = new RateLimiter(1500) // 1.5 seconds between calls
...
export class TldrawAgent {
	...
	async prompt(input: AgentInput) {
		...
		// Check if request was cancelled before continuing
		const activeRequest = this.$activeRequest.get()
		if (!activeRequest) {
			// Request was cancelled, don't continue
			console.log('[Agent] Request cancelled, stopping prompt chain')
			return
		}
...
If ANY requirement is missing, you MUST create additional actions to complete it. Do NOT finish until everything is verified.`],
		...
		// Check again if cancelled before processing scheduled request
		if (!this.$activeRequest.get()) {
			console.log('[Agent] Request cancelled before scheduled continuation')
			return
		}
	 ...
	 */
	cancel() {
		console.log('[Agent] Cancel called, aborting current request and clearing state')
		this.cancelFn?.()
		this.$activeRequest.set(null)
		this.$scheduledRequest.set(null)
		this.cancelFn = null
		// Also clear todo list to prevent continuation
		this.$todoList.set([])
	}
...
function requestAgent({ agent, request }: { agent: TldrawAgent; request: AgentRequest }) {
	...
	if (request.type === 'user') {
		...
		// Set chat origin to a free space on the canvas
		const freeSpaceOrigin = findFreeSpaceOnCanvas(editor)
		agent.$chatOrigin.set(freeSpaceOrigin)
	}
		...
		try {
			for await (const action of streamAgent({ prompt, signal })) {
				...
				if (cancelled || signal.aborted) {
					console.log('[Agent] Request cancelled, stopping action processing')
					break
				}
				
				editor.run(
					() => {
						// Check cancellation again inside editor.run
						if (cancelled || signal.aborted) {
							return
						}
						...
						// The the action is incomplete, save the diff so that we can reapply it in the future
				...
				// Check cancellation after processing each action
				if (cancelled || signal.aborted) {
					console.log('[Agent] Request cancelled, breaking from action loop')
					break
				}
			}
			
			// Don't wait for promises if cancelled
			if (!cancelled && !signal.aborted) {
				await Promise.all(actionPromises)
			}
		} catch (e) {
			if (cancelled || signal.aborted || e === 'Cancelled by user' || (e instanceof Error && e.name === 'AbortError')) {
				console.log('[Agent] Request cancelled during execution')
				return
			}
			agent.onError(e)
		}
	...
	const cancel = () => {
		...
		console.log('[Agent] Cancel function called, aborting request')
	}
...
async function* streamAgent({
	...
	// Rate limit: Wait before making the API call to avoid hitting rate limits
	await apiRateLimiter.waitForNextCall()
	...
	if (!res.ok) {
		...
		console.error(`Stream API error: ${res.status} ${res.statusText}`, errorText);
		throw new Error(`Stream API failed: ${res.status} ${res.statusText} - ${errorText}`);
	}

	if (!res.body) {
		console.error('Stream API returned no body');
		throw new Error('No body in response from stream API');
	}
	...
	try {
		while (true) {
			...
			if (signal.aborted) {
				console.log('[Agent] Stream cancelled via AbortSignal, stopping stream')
				break
			}
			...
			if (signal.aborted) {
				console.log('[Agent] Stream cancelled after read, stopping stream')
				break
			}
			...
			for (const action of actions) {
				// Check cancellation during action processing
				if (signal.aborted) {
					console.log('[Agent] Stream cancelled during action parsing, stopping')
					break
				}
				...
				if (match) {
					try {
						...
						// If cancelled, don't throw, just break
						if (signal.aborted) {
							break
						}
...
/**
 * Find a free space on the canvas for new drawings.
 * Returns the origin point where new shapes should be placed.
 */
function findFreeSpaceOnCanvas(editor: Editor): VecModel {
	...
	if (allShapes.length === 0) {
		// Canvas is empty, start in center of viewport
		return {
			x: viewportBounds.midX,
			y: viewportBounds.midY,
		}
	}

	// Calculate bounding box of all existing shapes
	let minX = Infinity
	let minY = Infinity
	let maxX = -Infinity
	let maxY = -Infinity
	...
	// Add padding around existing content to avoid overlaps
	const padding = 300 // pixels
	
	// Calculate potential placement locations (prioritized order)
	const candidates: VecModel[] = [
		{ x: maxX + padding, y: minY }, // Right side
		{ x: minX, y: maxY + padding }, // Below
		{ x: minX, y: minY - padding - 300 }, // Above (accounting for content height)
		{ x: minX - padding - 300, y: minY }, // Left (accounting for content width)
	]

	// Find the first candidate that doesn't overlap significantly with existing shapes
	// For now, we'll use a simple approach: check if the candidate point is far enough
	// from all existing shape bounds
	let bestOrigin = candidates[0] // Default to right side
	
	for (const candidate of candidates) {
		let hasOverlap = false
		const minDistance = 250 // Minimum distance from any existing shape
		
		for (const shape of allShapes) {
			const bounds = editor.getShapePageBounds(shape)
			if (!bounds) continue
			
			// Check if candidate is too close to this shape
			const distanceX = Math.abs(candidate.x - bounds.midX)
			const distanceY = Math.abs(candidate.y - bounds.midY)
	...
	// Ensure the origin is reasonably within or near viewport
	const viewportMargin = 200
	if (bestOrigin.x < viewportBounds.minX - viewportMargin) {
		bestOrigin.x = viewportBounds.maxX + padding
	}
	if (bestOrigin.y < viewportBounds.minY - viewportMargin) {
		bestOrigin.y = viewportBounds.minY
	}
