...
export class PenActionUtil extends AgentActionUtil<PenAction> {
	...
	override sanitizeAction(action: Streaming<PenAction>, helpers: AgentHelpers) {
		if (!action.points) {
			// Return null to reject the action if there are no points
			return null
		}
		...
		const validPoints = action.points
			...
			.filter((v) => v !== null) as VecModel[]

		// For complete actions, require at least 2 points (minimum for a polyline)
		if (action.complete && validPoints.length < 2) {
			// Reject incomplete pen actions - they need at least 2 points
			return null
		}
	...
	override applyAction(action: Streaming<PenAction>, helpers: AgentHelpers) {
		...
		// Require at least 2 points for a valid draw shape (polyline needs at least 2 points)
		// For incomplete actions, we'll allow 1 point temporarily, but skip creation until we have at least 2
		if (action.complete && action.points.length < 2) {
			console.warn('[PenActionUtil] Rejected pen action: requires at least 2 points, got', action.points.length)
			return
		}
		
		// For incomplete actions with less than 2 points, don't create the shape yet
		if (!action.complete && action.points.length < 2) {
			return
		}
		...
		// Process points between segments
		...
		// Add the last point if we have at least 2 points
		if (action.points.length >= 2) {
			const lastPoint = action.points[action.points.length - 1]
			points.push(lastPoint)
		}

		// Ensure we have at least 2 points in the final array before creating the shape
		// This is required for a valid polyline (Polyline2d requires at least 2 points)
		if (points.length < 2) {
			console.warn('[PenActionUtil] Rejected pen action: processed points array has less than 2 points, got', points.length, 'from', action.points.length, 'input points')
			return
		}

		// Map points to segment points (relative to minX/minY)
		...
		// Final validation: ensure segments have at least 2 points (required for Polyline2d)
		if (segmentPoints.length < 2) {
			console.warn('[PenActionUtil] Rejected pen action: segment points array has less than 2 points, got', segmentPoints.length)
			return
		}
