...
export class CreateActionUtil extends AgentActionUtil<CreateAction> {
	...
	override sanitizeAction(action: Streaming<CreateAction>, helpers: AgentHelpers) {
		...
		// Ensure the shape has a shapeId (required field)
		if (!shape.shapeId || typeof shape.shapeId !== 'string') {
			// Generate a default shapeId if missing
			const timestamp = Date.now()
			const random = Math.random().toString(36).substring(2, 9)
			shape.shapeId = `shape-${timestamp}-${random}`
		}
